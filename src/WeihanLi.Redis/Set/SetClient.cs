...
namespace WeihanLi.Redis
{
    internal class SetClient<T> : BaseRedisClient, ISetClient<T>
    {
        ...
        public SetClient(string setName) : base(LogHelper.GetLogHelper<SetClient<T>>(), new RedisWrapper("Set"))
        {
        ...
        public T[] Combine(SetOperation operation, string another, CommandFlags flags = CommandFlags.None) => Wrapper.Unwrap<T>(Wrapper.Database.SetCombine(operation, _keyName, $"{Wrapper.KeyPrefix}/{another}", flags));

        public T[] Combine(SetOperation operation, string[] keys, CommandFlags flags = CommandFlags.None)
        {
            var redisKeys = keys.Select(k => $"{Wrapper.KeyPrefix}/{k}").ToList();
        ...
        public long CombineAndStore(SetOperation operation, string destination, string another,
            CommandFlags flags = CommandFlags.None) => Wrapper.Database.SetCombineAndStore(operation, $"{Wrapper.KeyPrefix}/{destination}",
            _keyName, $"{Wrapper.KeyPrefix}/{another}", flags);

        public long CombineAndStore(SetOperation operation, string destination, string[] keys, CommandFlags flags = CommandFlags.None)
        {
            var redisKeys = keys.Select(k => $"{Wrapper.KeyPrefix}/{k}").ToList();
            redisKeys.AddIfNotContains(_keyName);
            return Wrapper.Database.SetCombineAndStore(operation, $"{Wrapper.KeyPrefix}/{destination}",
                redisKeys.Select(_ => (RedisKey)_).ToArray(), flags);
        }

        public Task<long> CombineAndStoreAsync(SetOperation operation, string destination, string another, CommandFlags flags = CommandFlags.None) => Wrapper.Database.SetCombineAndStoreAsync(operation, $"{Wrapper.KeyPrefix}/{destination}",
            _keyName, $"{Wrapper.KeyPrefix}/{another}", flags);
        ...
        public async Task<long> CombineAndStoreAsync(SetOperation operation, string destination, string[] keys, CommandFlags flags = CommandFlags.None)
        {
            var redisKeys = keys.Select(k => $"{Wrapper.KeyPrefix}/{k}").ToList();
            redisKeys.AddIfNotContains(_keyName);
            return await Wrapper.Database.SetCombineAndStoreAsync(operation, $"{Wrapper.KeyPrefix}/{destination}",
                redisKeys.Select(_ => (RedisKey)_).ToArray(), flags);
        }

        public async Task<T[]> CombineAsync(SetOperation operation, string another, CommandFlags flags = CommandFlags.None)
            => Wrapper.Unwrap<T>(await Wrapper.Database.SetCombineAsync(operation, _keyName, $"{Wrapper.KeyPrefix}/{another}", flags));
        ...
        public bool Move(string destination, T value, CommandFlags flags = CommandFlags.None) => Wrapper.Database.SetMove(_keyName, $"{Wrapper.KeyPrefix}/{destination}", Wrapper.Wrap(value), flags);

        public Task<bool> MoveAsync(string destination, T value, CommandFlags flags = CommandFlags.None) => Wrapper.Database.SetMoveAsync(_keyName, $"{Wrapper.KeyPrefix}/{destination}", Wrapper.Wrap(value), flags);
        ...
        public long SortAndStore(string destination, long skip = 0, long take = -1, Order order = Order.Ascending,
            SortType sortType = SortType.Numeric, T by = default(T), T[] get = null, CommandFlags flags = CommandFlags.None) => Wrapper.Database.SortAndStore(_keyName, $"{Wrapper.KeyPrefix}/{destination}", skip, take, order, sortType, Wrapper.Wrap(by),
            get?.Select(_ => Wrapper.Wrap(_)).ToArray(), flags);

        public Task<long> SortAndStoreAsync(string destination, long skip = 0, long take = -1, Order order = Order.Ascending, SortType sortType = SortType.Numeric, T by = default(T), T[] get = null, CommandFlags flags = CommandFlags.None) => Wrapper.Database.SortAndStoreAsync(_keyName, $"{Wrapper.KeyPrefix}/{destination}", skip, take, order, sortType, Wrapper.Wrap(by),
            get?.Select(_ => Wrapper.Wrap(_)).ToArray(), flags);
